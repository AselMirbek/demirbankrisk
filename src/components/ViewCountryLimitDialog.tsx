...
const ViewCountryLimitDialog: React.FC<Props> = ({ open, onOpenChange, country }) => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[80vh] overflow-auto">
        <DialogHeader>
          <DialogTitle>View Country — {country.name} ({country.code})</DialogTitle>
        </DialogHeader>
        ...
        <div className="grid grid-cols-1 gap-4 mt-4">
          <div>
            ...
            <div className="mt-2 space-y-2">
              <div>Code: <strong>{country.code}</strong></div>
              <div>Country: <strong>{country.name}</strong></div>
          ...
          <div>
            <h4 className="font-medium mt-4">Active Request (if any)</h4>
            {country.pending ? (
              <div className="mt-2 space-y-2">
                <div>Old Limit: <strong>{country.pending.oldLimit}</strong></div>
                <div>New Limit: <strong>{country.pending.newLimit}</strong></div>
                <div>Old Valid Until: <strong>{country.pending.oldValidUntil}</strong></div>
                <div>New Valid Until: <strong>{country.pending.newValidUntil}</strong></div>
                <div>Old Protocol: <strong>{country.pending.oldProtocol}</strong></div>
                <div>New Protocol: <strong>{country.pending.newProtocol}</strong></div>
                <div>Requested By: <strong>{country.pending.requestedBy}</strong></div>
                <div>Requested At: <strong>{country.pending.requestedAt}</strong></div>
                <div>Status: <strong>{country.pending.status}</strong></div>
              </div>
            ) : (
          ...
          <div className="mt-6">
            <h4 className="font-medium">History (all records)</h4>
